...
summary: 자바스크립트의 에러 처리 메커니즘인 예외(Exception)에 대해 알아봅니다. 클래스와 인스턴스의 개념부터 try-catch를 사용한 예외 처리, 스택 트레이스 읽는 법까지, 안정적인 코드를 만드는 핵심 기술을 배웁니다.
tags:
    - 자바스크립트 예외
    - try catch
    - throw
...
# 웹 개발 배우기 10편 - 예측 불가능한 에러를 우아하게 다루는 법, 자바스크립트 예외 처리
...
첫째, 클래스는 일반적인 함수 호출 문법 앞에 `new` 키워드를 붙여서 호출합니다.

둘째, 클래스는 항상 '객체(object)'를 반환하는데, 이 객체들을 해당 클래스의 '인스턴스(instance)'라고 부릅니다.

아래 두 표현식은 거의 같다고 볼 수 있습니다.
...
하지만 `new Array()` 방식에는 몇 가지 까다로운 점이 있어서, 실제로는 거의 항상 배열 리터럴 `[]`을 사용하는 것이 좋습니다.

마찬가지로 `new Object()`는 `{}`와 같습니다.

### `instanceof`로 클래스 인스턴스인지 확인하기

어떤 값이 특정 클래스의 '인스턴스(instance)'인지 확인하고 싶을 때 `instanceof` 연산자를 사용할 수 있거든요.

값 `v`가 클래스 `C`의 인스턴스인지 확인하려면 `v instanceof C`와 같이 사용하면 됩니다.
...
`error.stack` 프로퍼티는 `Error` 인스턴스가 코드의 어느 위치에서 생성되었는지 알려주는 아주 유용한 정보거든요.
...
이때 스택 트레이스 정보를 얻기 위해 `Error`의 인스턴스를 사용하는 것이 일반적입니다.
...
보통 예외가 던져지면 현재 코드의 실행은 그 즉시 멈추게 됩니다.
...
이름만으로도 어떤 종류의 에러인지 짐작할 수 있고, 에러의 종류를 더 쉽게 구분할 수 있게 해줍니다.

자주 사용되는 서브클래스는 다음과 같습니다.

`RangeError`는 값이 허용된 범위를 벗어났을 때 발생합니다.

`ReferenceError`는 유효하지 않은 참조를 했을 때 발생합니다.

`SyntaxError`는 코드의 문법이 잘못되었을 때 발생합니다.

`TypeError`는 값이 예상된 타입이 아닐 때 발생합니다.

`URIError`는 URI 관련 함수가 잘못 사용되었을 때 발생합니다.
...
이를 통해 우리는 에러를 '잡아서(catch)' 적절하게 처리할 수 있습니다.
...
A 함수가 B 함수를 호출하고, B 함수가 다시 C 함수를 호출하는 식으로 말이죠.

이렇게 깊게 중첩된 함수 안에서 예외가 발생하면, 그 예외는 자신을 호출한 함수로 계속해서 전달되는데요.

결국 `catch` 블록을 만나 처리되거나, 프로그램의 최상위 레벨에 도달해 프로그램을 중단시키게 됩니다.

다시 말해, 아래 `try-catch` 구문은 `someFunc()` 함수를 실행하는 동안 발생하는 '모든' 예외를 잡아낼 수 있습니다.
...
물론 더 완성도 높은 프로그램을 만들게 되면, 적절한 위치에서 예외를 잡아서 사용자에게 친절한 에러 메시지를 보여주는 등의 처리를 해야 합니다.
