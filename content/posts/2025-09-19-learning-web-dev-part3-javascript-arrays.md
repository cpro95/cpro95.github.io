...
배열에 몇 개의 요소가 들어있는지는 `.length`로 확인할 수 있거든요.
...
아래 코드에서 `num` 변수에 담긴 숫자 `0`이 바뀌는 게 아니라, `num + 1`의 결과인 새로운 숫자 `1`이 `num`이라는 상자에 새로 담기는 원리입니다.
...
이런 주석들은 자바스크립트가 코드를 실행할 때 완전히 무시합니다.
