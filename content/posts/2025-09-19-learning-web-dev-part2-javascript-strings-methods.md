...
tags:
  ...
  - 이벤트 리스너
  - DOM 조작
summary: 웹 개발 배우기 시리즈 2편입니다. 자바스크립트의 문자열과 메소드 개념을 배우고, 클릭 카운터와 온도 변환기 같은 간단한 웹 앱을 직접 만들어보며 이벤트 처리 방법을 익힙니다.
---
...
'데이터 타입'이란 '비슷한 종류'의 값들을 모아놓은 집합이라고 할 수 있는데요.
...
이건 우리가 이전에 봤던 숫자 `123.0`과 `123`이 문법은 달라도 결국 같은 값을 나타내는 것과 비슷한 원리입니다.

## 문자열의 길이

우리는 `.length`라는 프로퍼티를 사용해서 문자열이 몇 개의 문자로 이루어져 있는지 알아낼 수 있는데요.
...
변수에 문자열을 저장하고 플러스(+) 연산자로 이어 붙일 수도 있거든요.
...
자바스크립트에서는 변수 안에 또 다른 변수가 들어갈 수 있는데요.

이렇게 변수가 다른 변수를 담고 있는 구조를 '객체(object)'라고 부릅니다.

그리고 객체 안에 들어 있는 변수들은 그 객체의 '프로퍼티(property)'라고 하거든요.

예를 들어 자바스크립트에는 `Math`라는 미리 정의된 객체가 있고, 그 안에는 원주율 값을 가진 `PI`라는 프로퍼티가 있습니다.
...
프로퍼티는 함수를 담을 수도 있는데, 우리는 이 함수를 호출할 수 있습니다.
...
`Math.sqrt`는 숫자의 제곱근을 계산하는 함수를 담고 있는 프로퍼티인데요.
...
HTML과 자바스크립트 코드는 텍스트 파일에 저장되거든요.
...
이제 브라우저 콘솔에서 벗어나 실제 웹 앱의 세계로 들어가 볼까요.
...
그리고 `console.log(message);`는 `message`라는 변수에 담긴 값을 콘솔에 출력하라는 명령어입니다.

이 HTML 파일을 브라우저에서 열고 개발자 도구 콘솔을 확인하면 'Hello Terry!'라는 메시지가 출력된 것을 볼 수 있을 겁니다.

## 프로젝트 log-clicks.html

이번에는 사용자가 링크를 클릭할 때마다 자바스크립트 코드를 실행시켜 볼 건데요.

아래와 같이 `id`가 'link'인 `< a >` 태그가 있다고 가정해 봅시다.
...
2번째 줄의 `document.querySelector('#link')`는 HTML 문서에서 `id`가 'link'인 요소를 찾아오라는 뜻입니다.

그리고 3번째 줄의 `addEventListener`는 '이벤트 리스너'를 추가하는 메소드인데요.
...
이제 우리는 숫자와 문자열을 모두 알게 되었는데요.

그렇다면 숫자 `123`과 문자열 `'123'`은 무슨 차이가 있을까요?

`123`은 숫자라서 산술 연산을 할 수 있지만, `'123'`은 문자열이라 다른 문자열과 연결하는 등의 작업만 가능합니다.
...
다행히 자바스크립트는 이 둘을 서로 변환할 수 있는 함수를 제공하는데요.
...
지금까지는 콘솔에 정보를 표시했지만, 일반 사용자들이 콘솔을 열어보지는 않겠죠.
...
그리고 C라인에서는 그 숫자를 화씨로 변환한 뒤, 다시 문자열로 바꿔 `fahrenheit` 입력 필드의 `.value`에 넣어주는 거죠.
