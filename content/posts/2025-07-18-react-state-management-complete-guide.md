...
tags:
  ...
  - Context API
  - Zustand
summary: React의 상태 관리 기법을 useState, useReducer, Context API부터 Zustand, TanStack Query와 같은 최신 라이브러리까지 종합적으로 다룹니다. 클라이언트 상태와 서버 상태를 구분하여 올바른 도구를 선택하는 방법을 배워보세요.
---
# React 상태 관리 완벽 가이드 useState부터 TanStack Query까지
