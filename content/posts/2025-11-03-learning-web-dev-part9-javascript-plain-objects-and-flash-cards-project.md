...
summary: 자바스크립트 객체의 기본 개념부터 프로퍼티 사용법, 그리고 플래시 카드 앱 실습까지 알아봅니다. 모델과 뷰 개념을 통해 데이터와 UI를 분리하는 방법도 함께 배웁니다.
tags:
    - 자바스크립트 객체
    - 객체 리터럴
    - 플래시 카드
...
# 웹 개발 배우기 9편 - 자바스크립트 객체의 모든 것, 플래시 카드 앱으로 정복하기
...
이번 프로젝트는 무작위로 플래시 카드를 보여주는 앱인데요.
...
const goToNextCard = () => {
  ...
  if (currentIndex >= data.length) {
    startNewRound(); // 마지막 카드를 넘기면 새 라운드 시작
  } else {
    showCard(currentIndex);
  }
}
...
export const shuffleArray = (arr) => {
  for (let current = arr.length - 1; current >= 1; current--) {
    ...
    // `current`와 `randomPick` 인덱스의 요소를 서로 교환
