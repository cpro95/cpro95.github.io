...
document.addEventListener('DOMContentLoaded', () => {
  ...
  svgElem.className = 'svg-tip svg-tip-one-line';
  ...
  try {
    ...
    dom.innerHTML = "<p>Error: Could not load post activity data. Check for special characters in post titles.</p>";
    return;
  }
...
function switchYear(year) {
  ...
  for (const post of posts) {
    const monthKey = `${post.date.getFullYear()}-${post.date.getMonth()}`;
    if (!monthlyGroups[monthKey]) {
      monthlyGroups[monthKey] = [];
    }
    monthlyGroups[monthKey].push(post);
  }
  ...
  for (const key of sortedMonthKeys) {
    ...
    const monthlyPosts = monthlyGroups[key];
    const node = document.createElement('div');
    node.innerHTML = monthly(yearStr, Number(monthStr), monthlyPosts);
...
function monthly(year, month, monthPosts) {
  let liHtml = '';
      ...
      <div class="TimelineItem">
        ...
        <div class="TimelineItem-body">
          <details class="Details-element details-reset" open>
            ...
            <div><ul class="list-style-none mt-1">${liHtml}</ul></div>
          </details>
        </div>
      </div>
    </div>
...
function graph(year, posts, startDate, endDate) {
  ...
  for (let i = 0; i < 53; i++) {
    ...
    for (let j = 0; j < 7; j++) {
      ...
      let c = count[dataDate] || 0;
      let color;
      switch (c) {
        case 0: color = "var(--color-calendar-graph-day-bg)"; break;
        case 1: color = "var(--color-calendar-graph-day-L1-bg)"; break;
        case 2: color = "var(--color-calendar-graph-day-L2-bg)"; break;
        case 3: color = "var(--color-calendar-graph-day-L3-bg)"; break;
        default: color = "var(--color-calendar-graph-day-L4-bg)";
      }
      html += `<rect class="day" width="11" height="11" x="${16 - i}" y="${j * 15}" fill="${color}" onmouseover="svgTip(this, ${c}, '${dataDate}')" onmouseleave="hideTip()"></rect>`;
    }
    html += '</g>';
  }
  if (monthPos[1] - monthPos[0] < 2) monthPos[0] = -1;
  for (let i = 0; i < monthPos.length; i++) {
    const month = monthPos[i];
    if (month === -1) continue;
    html += `<text x="${15 * month + 16}" y="-9" class="month">${months[(i + startDate.getMonth()) % 12]}</text>`;
  }
  html += `<text text-anchor="start" class="wday" dx="-10" dy="25">Mon</text><text text-anchor="start" class="wday" dx="-10" dy="56">Wed</text><text text-anchor="start" class="wday" dx="-10" dy="85">Fri</text>`;
  document.querySelector('#graph-svg').innerHTML = html;
}

function svgTip(elem, count, dateStr) {
  if (window.screen.width < 768 || !svgElem) return;
  const rect = getCoords(elem);
  const date = new Date(dateStr);
  const dateFmt = `${months[date.getMonth()]} ${date.getDate()}, ${date.getFullYear()}`;
  svgElem.innerHTML = count ? `<strong>${count} posts</strong> on ${dateFmt}` : `<strong>No posts</strong> on ${dateFmt}`;
