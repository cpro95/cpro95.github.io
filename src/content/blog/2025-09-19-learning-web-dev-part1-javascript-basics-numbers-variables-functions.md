...
예를 들어, 자바스크립트는 이렇게 숫자를 더할 수 있거든요.<br /><br />
...
이제부터 콘솔 상호작용은 아래와 같은 방식으로 표시할 건데요.<br /><br />
...
뺄셈 연산자 `-` (하이픈)를 사용하면 숫자를 뺄 수 있거든요.<br /><br />
...
곱셈 연산자 `*` (별표)는 숫자를 곱하는 데 사용됩니다.<br /><br />
...
나눗셈 연산자 `/` (슬래시)를 쓰면 숫자를 나눌 수 있는데요.<br /><br />
...
물론 더 긴 숫자 표현식도 작성할 수 있는데요.<br /><br />
...
변수란 값을 담을 수 있는 '이름이 붙은 상자'라고 생각하시면 됩니다.<br /><br />
...
첫 번째 줄에서 `myNumber`라는 변수를 만들었습니다.<br /><br />
...
그래서 앞으로는 세미콜론으로 끝나는 콘솔 입력의 결과값(`undefined`)은 생략하고 보여드리겠습니다.<br /><br />
...
예를 들어 숫자로 시작하는 잘못된 변수 이름을 사용해 보겠습니다.<br /><br />
...
변수에 담긴 내용을 가져오는 것뿐만 아니라, 그 내용을 바꿀 수도 있는데요.<br /><br />
...
처음에는 `value`라는 변수를 `0`으로 초기화했지만, 나중에 그 값을 `3`으로 변경했습니다.<br /><br />
이때 `let` 없이 할당 연산자 `=`를 사용했거든요.<br /><br />
할당 연산자의 오른쪽에는 어떤 표현식이든 올 수 있습니다.<br /><br />
...
이 때문에 수학적으로는 이상해 보이는 이런 문장도 가능한데요.<br /><br />
...
이렇게 하면 실수로 값을 바꾸려고 할 때 자바스크립트가 경고를 해줍니다.<br /><br />
...
아래 예제에서 `timesThree`라는 변수가 바로 함수입니다.<br /><br />
...
첫 번째 줄에서 `timesThree`라는 함수를 만들었고, 두 번째 줄에서는 `2`라는 값을 넣어 함수를 호출했습니다.<br /><br />
...
자바스크립트는 대부분 줄 바꿈을 무시하기 때문에, 사람이 읽기 쉽게 코드를 여러 줄로 나눠 쓸 수 있습니다.<br /><br />
...
다음 함수는 섭씨 온도를 화씨 온도로 변환하는 함수인데요.<br /><br />
...
이 코드를 콘솔에 붙여넣고 함수를 호출해 볼 수 있거든요.<br /><br />
